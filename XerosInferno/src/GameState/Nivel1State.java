...
import Entidad.Enemigos.Woles;
import Entidad.Explosion;
import Entidad.HUD;
import Entidad.Osmy;
import Entidad.Portal;
...
public class Nivel1State extends GameState {
    ...
    //Creando las arrayList de los enemigos y efectos especiales
    private ArrayList<Enemigo> arrEnemigos;
    private ArrayList<Explosion> arrExplosiones;
    
    //prueba de fuego
    //private Fuego eneFuego;
    ...
    public void init() {
        ...
        //llamando a la funcion que pone a los enemigos en el mapa
        populateEnemies();
        
        //Inicializando el array de arrExplosiones
        arrExplosiones = new ArrayList<Explosion>();
        ...
        //INicializando el portal de teletransportacion
        portal = new Portal(tileMap);
        portal.setPosition(450, 510);
    ...
    private void populateEnemies() {
        ...
        for ( int i = 0; i < Sluggers.length; i++) {
            eneSlugger = new SpiritFire(tileMap, osmy);
            eneSlugger.setPosition(Sluggers[i].x, Sluggers[i].y);
            arrEnemigos.add(eneSlugger);
        }
        
        Point [] Woles = new Point[]{
            new Point(3100, 540),
            new Point(3200, 540),
            new Point(3300, 540),
            new Point(3400, 540),
        ...
        for ( int i = 0; i < Woles.length; i++) {
            eneWoles = new Woles(tileMap, osmy);
            eneWoles.setPosition(Woles[i].x, Woles[i].y);
            arrEnemigos.add(eneWoles);
        }
    ...
    public void draw(Graphics2D g) {
        ...
        //dibujar las arrExplosiones
        for (int i=0; i < arrExplosiones.size(); i++) {
            arrExplosiones.get(i).setMapPosition((int)tileMap.getX(), 
                    (int)tileMap.getY());
            arrExplosiones.get(i).draw(g);
        }
    ...
    public void update() {
        ...
        //Checar si osmy esta dentro del portal de teletransportacion
        if(portal.intersects(osmy)) {
            boolEventoFinish = boolBloquearControl = true;
        }
	...
	if(osmy.getHealth() == 0 || osmy.getY() > tileMap.getHeight()) {
        ...
        if(boolEventoFinish) eventFinish();
        ...
        for (int i=0; i < arrEnemigos.size(); i++) {
            ...
            if(e.isDead()) {
                ...
                i--;
                arrExplosiones.add(new Explosion(e.getX(), e.getY()));
            }
        }
        
        //haciendo update a las arrExplosiones
        for (int i=0; i < arrExplosiones.size(); i++) {
            Explosion e = arrExplosiones.get(i);
            e.update();
            if(e.shouldRemove()) {
                arrExplosiones.remove(i);
                i--;
            }
        }
        //Haciendo update al portal
        portal.update();
    }
    ...
    public void keyPressed(int k) {
        ...
        if(k == KeyEvent.VK_SPACE && !osmy.boolEarth) {
            osmy.setJumping(true);
        }
        if(k == KeyEvent.VK_F && osmy.boolFire) {
            osmy.setFiring();
        }
    }
    ...
    */
	...
	iEventoConta++;
	if(iEventoConta == 1) {
            osmy.setTeleporting(true);
	}
        else if(iEventoConta == 60) {
            arrTransiciones.clear();
            arrTransiciones.add(new Rectangle(
                GamePanel.WIDTH / 2, GamePanel.HEIGHT / 2, 0, 0));
	}
	else if(iEventoConta > 60) {
            arrTransiciones.get(0).x -= 6;
            arrTransiciones.get(0).y -= 4;
            arrTransiciones.get(0).width += 12;
            arrTransiciones.get(0).height += 8;
	}
        if(iEventoConta == 180) {
